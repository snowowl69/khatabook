/**
 * Khata - Database Migration Runner
 *
 * SQL is embedded directly as template literals to avoid
 * file-system path issues with electron-vite bundling.
 */
...
/* ── Migration 002: Bill Print, Vehicle & Bank Details ──────────── */
const MIGRATION_002 = `
-- Add vehicle_number to bills
ALTER TABLE bills ADD COLUMN vehicle_number TEXT;

-- Print audit log: tracks every print event
CREATE TABLE IF NOT EXISTS bill_print_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  bill_id INTEGER NOT NULL REFERENCES bills(id),
  user_id INTEGER REFERENCES users(id),
  printed_at TEXT NOT NULL DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_bill_print_log_bill ON bill_print_log(bill_id);

-- Bank & QR settings for invoice printing
INSERT OR IGNORE INTO settings (key, value, category) VALUES
  ('shop_email', '""', 'general'),
  ('shop_state_code', '""', 'general'),
  ('bank_account_name', '""', 'billing'),
  ('bank_account_number', '""', 'billing'),
  ('bank_ifsc', '""', 'billing'),
  ('bank_name', '""', 'billing'),
  ('bank_upi_id', '""', 'billing'),
  ('qr_image_path', '""', 'billing'),
  ('invoice_terms', '""', 'billing');
`

/** Ordered list of migrations. Add new entries at the bottom. */
const MIGRATIONS: { name: string; sql: string }[] = [
  { name: '001_initial.sql', sql: MIGRATION_001 },
  { name: '002_bill_print_vehicle_bank.sql', sql: MIGRATION_002 },
]

/**
 * Runs all pending SQL migrations against the provided database.
 */
export function runMigrations(db: Database.Database): void {
  ...
  if (appliedCount === 0) {
    ...
    console.log(`[Migrations] Done — applied ${appliedCount} migration(s)`)
  }
}
